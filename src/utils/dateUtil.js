...
function getLunarDate(date) {
	...
	let year=cyclical(currentYear - 1900 + 36) + '年 ';
	let month=cDay(lDObj.month, lDObj.day,date).month ;
	let day=cDay(lDObj.month, lDObj.day,date).day;
...
function getHoroscope(date) {
	var SDate=new Date(date.replace(/-/ig, '/')); 
...
function getCurrentDate(){
	...
	const day=date.getDate()<date.getDate()?'0'+date.getDate():date.getDate();
	const week =date.getDay();
	const dateStr=year+'-'+month+'-'+day;
	let lunarDate=getLunarDate(dateStr);
	let horoscope=getHoroscope(dateStr);
	return{
		year:year,
		month:month,
		day:day,
